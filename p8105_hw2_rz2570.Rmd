...
male_name_df = filter(names_df, gender == "MALE") %>% 
  arrange(-count)

jayden_df = filter(
  male_name_df,
  childs_first_name == "JAYDEN") %>% 
  select(-c(childs_first_name, gender, count)) %>% 
  arrange(year_of_birth)

jayden_table = pivot_wider(
  jayden_df,
  names_from = "year_of_birth",
  values_from = "rank"
)
...
ggplot(plot_data, aes(x = rank, y = count)) + 
  ...
  theme_minimal() +
  labs(
    title = "Names rank against count for male white non-hispanic children born in 2016",
    x = "Rank",
    y = "Count",
